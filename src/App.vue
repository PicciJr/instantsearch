<template>
  <div id="app">
    <form action>
      <input type="text" placeholder="Enter your search terms..." v-model="userInput" />
      <p v-for="art in filteredArticles" :key="art">{{art.title}}</p>
    </form>
  </div>
</template>
...
export default {
  ...
  computed: {
    filteredArticles: function() {
      ...
      let searchString = this.userInput.trim().toLowerCase();

      if (!searchString) {
        return articles_array; //si es vacio, ya salgo.
      }
